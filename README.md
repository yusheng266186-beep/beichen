...
1. **思考强度三档**：设置面板可选 省时 / 标准 / 深思（默认标准）；客户端只传枚举，思考预算由服务端换算（标准沿用环境变量口径，省时减半、深思加倍）。设置面板给出各档预估星图生成时间。
2. **思考低语优化 + 开关**：低语行改为恒定尾窗与固定行宽，不再忽长忽短；设置面板可关（默认开）。
3. **新密钥覆盖旧存档**：每次输入新验证码自动开始全新谈心，旧对话存档自动覆盖；历次星图档案保留在本机。
4. **值班哨兵**：仓库内置 GitHub Actions 每 5 分钟拨测 `/readyz`，失败自动邮件告警。

详细的版本、部署和验收记录见 [CHANGELOG.md](CHANGELOG.md)。
...
| `POST` | `/chat/completions` | 提交现有页面格式的流式聊天请求；`effort?` 可选枚举 `eco/standard/deep`（思考强度，缺省 standard，预算由服务端换算） |
