...
本次是后端迭代 **v2.7.0-stateful**。页面仍然是 **v2.6.19**，`index.html` 没有改动；两者可以直接配合工作。
...
页面原有的三种请求格式不需要改变：普通聊天使用 `max_tokens=5000`，星图使用 `max_tokens=16384`，确认请求使用空 JSON 加 `X-Request-ID`。后端会把高预算请求作为星图请求处理，并限制修复尝试次数；这保证旧页面不会因为这次安全修复失配。
...
| `GET` | `/healthz` | 存活检查，不需要秘钥 |
| `GET` | `/readyz` | 检查秘钥配置、Redis 和上游配置是否就绪 |
...
部署后先访问 `/healthz` 和 `/readyz`。只有 `/readyz` 返回 `ready: true`，再打开网页验证动态码。已有页面无需替换 `RELAY`；只有迁移到另一套云函数时，才按部署指南同步替换页面里的函数地址和 CSP 域名。
