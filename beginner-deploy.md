...
这份指南只覆盖正式库 `yusheng266186-beep/beichen` 的后端中转服务。它不要求改页面：当前页面已经使用后端兼容的三种请求格式。

这次后端版本是 `v2.7.0-stateful`，页面版本仍为 `v2.6.19`。真实的千帆 Key、动态码 seed、会话签名密钥和 Redis 密码只填写在腾讯云的加密环境变量里，绝不能写回 GitHub。
...
然后打开现有 Pages 页面，完成一次完整流程：输入动态码 → 普通聊天 → 生成星图 → 等待星图完成 → 确认次数。页面仍会访问现有的 `/verify`、`/chat/completions` 和 `/run/complete`，不需要额外加接口。

## 6. 页面地址什么时候需要改

本次后端修复不需要改 `index.html`。当前页面里的 `RELAY` 已指向正式函数。
