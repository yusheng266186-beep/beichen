...
这次后端和页面版本均为 `v2.8.1`。真实的千帆 Key、动态码 seed、会话签名密钥和 Redis 密码只填写在腾讯云的加密环境变量里，绝不能写回 GitHub。
...
然后打开本次更新后的 Pages 页面，先进入设置点击“检查连接”。正常情况下应显示“正常”，并看到后端 `v2.8.1`、页面 `v2.8.1`。如果显示“未就绪”，优先处理 Redis 或云端配置；如果显示“需同步”，说明页面和函数不是同一版本。
...
本次页面版本已经同步到 `v2.8.1`。当前页面里的 `RELAY` 仍指向正式函数，只有迁移到另一套云函数时才需要改地址。
